...
fish$max_age <- (-log(0.01)/fish$m) %>% floor()
...
f_v_m <- mean(cc_fishery$f) / fish$m
...
a <- n_at_age %>% 
  group_by(age) %>% 
  summarise(nc = sum(numbers_caught)) %>% 
  select(-age) %>% 
  as.matrix()

lh <- LIME::create_lh_list(linf = fish$linf,
                           vbk = fish$vbk,
                           lwa = fish$weight_a,
                           lwb = fish$weight_b,
                           ...
                           AgeMax = fish$max_age - 1)

lcomps <- LIME::AgeToLengthComp(lh =lh, tyears = 1, N_at = a, comp_sample = 412, sample_type = 1)
...
comp <- length_samples %>% 
  mutate(lime_numbers = lcomps$LF) %>% 
  gather('source','samples',-length_bin)

comp %>% 
  ggplot(aes(length_bin, samples, fill = source)) + 
...
lbspr_sim <- LBSPRsim(lbspr_fish, Control = list(modtype = 'absel'))
...
lbspr_lens <- length_samples %>%
  mutate(Length = length_bin + 0.5,
         year = 2017) %>% 
  select(Length, numbers, year) %>% 
  spread(year, numbers) %>% 
  as.matrix()
  ...
  lbspr_lengths@NYears <- dim(lbspr_lens)[2] - 1
  
  lbspr_lengths@Years <- 2017
    
  lbspr_fit <- LBSPRfit(lbspr_fish, lbspr_lengths,Control = list(modtype = 'absel', Nage = fish$max_age))
...
WHAT THE HELL IS HAPPENING HERE. Ok, one question is is your length sampling itself fucked. Let's try and use the LIME age to length function and see if those length samples look the same as yours. If it does, then it's either the difference in your population model or in your fishing model. 
